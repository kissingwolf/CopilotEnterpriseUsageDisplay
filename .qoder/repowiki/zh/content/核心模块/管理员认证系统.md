...
## 更新摘要
**所做更改**
- 新增用户预算页面访问控制章节，详细说明新功能的管理员认证机制
- 更新架构概览图，包含用户预算页面的认证流程
- 添加用户预算页面前端认证检测机制说明
- 更新依赖关系分析，反映新增的用户预算路由模块
...
**更新** 系统现已扩展支持新的用户预算页面，为预算管理功能提供完整的管理员访问控制，确保只有授权管理员可以访问和操作用户预算设置。
...
### 用户预算路由模块 (routes/user-budget.js)

**新增** 用户预算路由模块为预算管理功能提供完整的 API 支持：
...
2. **预算管理 API**
   - `GET /api/user-budgets` - 列出所有用户预算
   - `GET /api/user-budgets/:id` - 获取单个预算详情
   - `POST /api/user-budgets` - 创建新预算
   - `PATCH /api/user-budgets/:id` - 更新预算设置
   - `DELETE /api/user-budgets/:id` - 删除预算

3. **企业模式验证**
   - 确保在企业模式下运行
   - 验证 ENTERPRISE_SLUG 环境变量配置

**章节来源**
- [routes/user-budget.js:119-215](file://routes/user-budget.js#L119-L215)
...
### 用户预算前端界面 (public/userbudget.html & public/userbudget.js)

**新增** 用户预算页面提供完整的预算管理功能：

1. **预算管理页面** (`userbudget.html`)
   - 用户预算列表展示
   - SKU 筛选和搜索功能
   - 新建、编辑、删除操作按钮

2. **前端逻辑** (`userbudget.js`)
   - 自动检测管理员登录状态
   - 处理预算 CRUD 操作
   - 实时数据刷新和错误处理
...
participant UserBudgetRoute as 用户预算路由
...
Client->>Frontend : 访问用户预算页面
Frontend->>Server : GET /userbudget
Server->>AuthLib : requireAdminPage()
AuthLib-->>Server : 验证通过
Server-->>Frontend : 返回 userbudget.html
...
- [routes/user-budget.js:122-124](file://routes/user-budget.js#L122-L124)
...
participant UserBudgetJS as userbudget.js
...
Browser->>UserBudgetJS : 访问用户预算页面
UserBudgetJS->>API : GET /admin/session
API-->>UserBudgetJS : {authenticated : true}
UserBudgetJS->>Browser : 允许访问预算页面
```
...
**新增** 用户预算页面集成了完整的管理员访问控制机制：

1. **前端认证检测**
   - 页面加载时自动检查 `/admin/session` 端点
   - 未认证用户被重定向到登录页面
   - 支持 `next` 参数实现登录后跳转
...
H --> E
I[public/admin.js] --> H
J[routes/user-budget.js] --> G
J --> E
K[public/userbudget.js] --> J
end
subgraph "安全配置"
L[server.js] --> C
L --> E
M[scripts/hash-admin-password.js] --> A
end
```
...
- [routes/user-budget.js:11](file://routes/user-budget.js#L11)
...
**更新** 该认证系统现已扩展支持新的用户预算页面，为整个仪表盘提供了更加完善的安全基础，确保只有授权用户才能访问敏感的管理功能，同时保持了良好的用户体验和系统性能。新增的用户预算功能严格遵循相同的认证原则，确保预算管理操作的安全性和可控性。