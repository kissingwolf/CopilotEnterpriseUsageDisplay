...
- [routes/bill.js:13](file://routes/bill.js#L13)
...
- [lib/github-api.js:8-320](file://lib/github-api.js#L8-L320)
...
- 账单管理路由（/api/bill, /api/bill/refresh）
  - 功能：团队级月度账单计算、历史缓存与当前期状态判断、强制刷新整月
...
## 架构总览
路由层通过依赖注入与共享缓存（内存 Map、SQLite）实现高性能与一致性；服务层封装 GitHub API 并提供并发控制、重试、ETag 条件请求与 LRU 缓存；前端静态页面通过 SPA 方式访问。
...
Client->>Server : "GET /api/usage"
Server->>Usage : "路由匹配"
Usage->>Usage : "返回内存 state"
Usage-->>Client : "{ok, ranking, ...}"
Client->>Server : "POST /api/usage/refresh"
Server->>Usage : "路由匹配"
Usage->>GH : "构建查询参数/端点"
Usage->>GH : "并发拉取按日/周期"
GH-->>Usage : "返回 usageItems"
Usage->>Store : "保存/更新 SQLite"
Usage-->>Client : "{ok, ranking, mode, cacheHitRatio}"
```
...
## 结论
该路由系统通过“路由层 + 服务层 + 存储层”的清晰分层，结合多级缓存与并发控制，实现了高可用、可观测、可扩展的企业级用量与账单管理能力。建议在新增路由时遵循：
- 明确的参数校验与错误响应
- 优先使用 SQLite 缓存与条件请求
- 严格控制并发与单飞行去重
- 保持缓存键与 TTL 的一致性
