...
## 更新摘要
**变更内容**
- Insights路由层现在支持AI和基于规则的两种推荐合并机制
- 优先使用AI建议，同时保持确定性回退行为
- 新增智能洞察生成和融合算法
- 前端显示AI状态和回退信息
...
- **新增** 智能洞察路由：结合AI建议和规则引擎的混合推荐系统
- 认证路由：管理员登录、登出、会话管理
- 团队管理路由：团队列表、成员数缓存与刷新
- 成本中心路由：企业成本中心查询、预算与资源批量同步
- 用户映射路由：成员映射文件上传、重载与信息查询

**更新** 新增认证路由模块、AI Credits 账单路由支持和智能洞察路由，增强了路由系统的安全性、计费灵活性和智能化水平。
...
R6["routes/insights.js<br/>智能洞察路由"]
R7["routes/teams.js"]
R8["routes/user-mapping.js"]
end
subgraph "服务与存储"
...
LLM["lib/llm-insights.js<br/>AI洞察引擎"]
AGG["lib/insights-aggregator.js<br/>洞察聚合器"]
END
S --> A
S --> R1
S --> R2
...
- **新增** 智能洞察路由（/api/insights）
  - 功能：AI驱动的动态洞察生成，结合规则引擎的确定性建议
  - **核心特性**：AI优先策略、确定性回退、智能融合算法、状态追踪
...
## 架构总览
路由层通过依赖注入与共享缓存（内存 Map、SQLite）实现高性能与一致性；服务层封装 GitHub API 并提供并发控制、重试、ETag 条件请求与 LRU 缓存；前端静态页面通过 SPA 方式访问。**新增** 认证中间件确保受保护页面的安全访问，**新增** 智能洞察系统提供AI驱动的分析能力。
...
participant Insights as "routes/insights.js"
participant LLM as "lib/llm-insights.js"
participant Agg as "lib/insights-aggregator.js"
...
Client->>Server : "GET /api/insights?range=28"
Server->>Insights : "路由匹配"
Insights->>Agg : "构建洞察数据"
Insights->>LLM : "生成AI洞察"
LLM-->>Insights : "AI建议或空结果"
Insights->>Insights : "mergeInsights(规则+AI)"
Insights-->>Client : "融合后的洞察结果"
```
...
### 智能洞察路由（/api/insights）
- **新增** 设计要点
  - **双引擎融合**：结合AI动态洞察和规则引擎确定性建议
  - **优先级策略**：AI建议优先，规则建议作为确定性回退
  - **智能排序**：按严重程度（high/medium/low）统一排序
  - **状态追踪**：记录AI使用情况、回退原因和数据源
  - **容错机制**：AI失败自动回退到规则引擎，保证服务可用性
- **核心流程**
  1. 获取GitHub指标报告和模型使用数据
  2. 构建洞察数据负载（包含使用指标、代码生成数据等）
  3. 生成规则引擎洞察（确定性建议）
  4. 调用LLM生成AI洞察（动态建议）
  5. 合并两种洞察并按严重程度排序
  6. 返回融合结果和元数据
- API 定义
  - GET /api/insights?range=7|14|28|30|60|90
    - 响应字段：ok、data.meta（range、generatedAt、warnings、llm、source）、data.tabs（usage、codeGeneration）、data.insights[]
    - insights元素：type、title、severity、message、evidence、recommendation、source
- **AI状态追踪**
  - data.meta.llm.used：是否使用了AI洞察
  - data.meta.llm.reason：AI使用原因（ok/timeout/error/not_configured/empty）
  - data.meta.source：数据来源（github-reports/github-partial）
- 错误处理
  - GitHub API 异常：收集警告信息，部分数据可用时继续处理
  - LLM 调用失败：自动回退到规则引擎，不影响整体响应
  - 超时处理：15秒超时限制，避免长时间阻塞
...
Client->>Server : "GET /api/insights?range=28"
Server->>Insights : "路由匹配"
Insights->>Insights : "fetchLiveSources(range)"
Insights->>Agg : "buildInsightsPayload(sources)"
Agg-->>Insights : "data + ruleInsights"
Insights->>Rule : "generateInsightRecommendations()"
Rule-->>Insights : "ruleInsights[]"
Insights->>LLM : "generateLlmInsights(data)"
alt LLM成功
LLM-->>Insights : "llmInsights[]"
else LLM失败
LLM-->>Insights : "[] (used=false)"
end
Insights->>Insights : "mergeInsights(ruleInsights, llmInsights)"
Insights-->>Client : "融合后的洞察结果"
```
...
**章节来源**
- [routes/insights.js:105-137](file://routes/insights.js#L105-L137)
- [lib/llm-insights.js:126-130](file://lib/llm-insights.js#L126-L130)
...
Insights["routes/insights.js"] --> GH
Insights --> LLM["lib/llm-insights.js"]
Insights --> Agg["lib/insights-aggregator.js"]
...
- [routes/insights.js:1-6](file://routes/insights.js#L1-L6)
...
- 计算优化
  ...
  - **新增** 洞察融合优化：AI洞察失败快速回退，避免长时间等待
- I/O 与磁盘
  - SQLite WAL + NORMAL 提升并发写入稳定性
  - 定期清理旧数据与 ETag，防止膨胀
- **新增** AI洞察性能优化
  - 超时控制：15秒LLM调用超时，避免阻塞
  - 降级策略：AI失败不影响规则引擎执行
  - 状态追踪：记录AI使用情况用于性能监控
...
- [lib/llm-insights.js:87-88](file://lib/llm-insights.js#L87-L88)
...
- 常见错误与定位
  ...
  - **新增** AI洞察问题：检查 LLM_URL、LLM_API_KEY、LLM_MODEL 环境变量配置
- **新增** AI洞察状态诊断
  - 查看 data.meta.llm.used 判断是否使用了AI
  - 检查 data.meta.llm.reason 了解AI使用原因
  - 观察前端显示的 "AI: on" 或 "AI: fallback" 状态
- 统一错误处理
...
- [public/insights.js:230](file://public/insights.js#L230)

## 结论
该路由系统通过"路由层 + 服务层 + 存储层"的清晰分层，结合多级缓存与并发控制，实现了高可用、可观测、可扩展的企业级用量与账单管理能力。**新增** 的认证路由、AI Credits 账单路由和智能洞察路由进一步增强了系统的安全性、灵活性和智能化水平。特别是智能洞察系统通过AI优先策略和确定性回退机制，提供了更丰富的分析能力和更好的用户体验。建议在新增路由时遵循：
- 明确的参数校验与错误响应
- 优先使用 SQLite 缓存与条件请求
- 严格控制并发与单飞行去重
- 保持缓存键与 TTL 的一致性
...
- **新增** 智能洞察
  - GET /api/insights?range=7|14|28|30|60|90
    - 响应：包含 data.meta（range、generatedAt、warnings、llm、source）、data.tabs、data.insights[]
    - insights元素：type、title、severity、message、evidence、recommendation、source
- 账单管理
  - GET /api/bill?year&month
