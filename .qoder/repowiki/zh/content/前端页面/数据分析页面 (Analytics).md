...
#### 配额使用饼图实现
配额使用饼图是本次更新的核心功能，用于可视化展示用户在当前计费周期内的配额使用情况。该功能通过以下步骤实现：

1. **数据收集**：从 /api/analytics/quota-usage API 获取用户配额使用数据
2. **配额计算**：基于座位信息和计划配额计算每个用户的使用百分比
3. **分类统计**：将用户按使用百分比分为五个等级
4. **图表渲染**：使用 Chart.js Doughnut 图表展示分类结果

#### 配额使用分类标准
系统将用户配额使用情况按照以下标准进行分类：
- 配额使用小于 5%
- 配额使用 大于 5% 小于 50%
- 配额使用 大于 50% 小于 100%
- 配额使用 大于 100% 小于 200%
...
#### 团队筛选功能
配额使用分析支持按团队进行筛选，用户可以通过团队下拉菜单选择特定团队，系统会动态更新图表显示该团队的配额使用情况。

#### 用户列表展示
当用户点击饼图中的某个分类时，系统会显示该分类下所有用户的详细列表，包括用户名、团队和具体的使用百分比。

**章节来源**
- [analytics.html:80-93](file://public/analytics.html#L80-L93)
- [analytics.js:485-584](file://public/analytics.js#L485-L584)
- [analytics.js:603-627](file://public/analytics.js#L603-L627)
- [routes/analytics.js:96-179](file://routes/analytics.js#L96-L179)
- [helpers.js:258-281](file://lib/helpers.js#L258-L281)