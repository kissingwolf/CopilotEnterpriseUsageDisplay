...
**更新** 新增管理员控制台界面，提供完整的后台管理功能，包括登录认证、会话管理、权限控制和管理入口导航。
...
- [admin.html:1-205](file://public/admin.html#L1-L205)
...
- 全局样式
  ...
  - 组件：查询栏、表格、分页、模态框、Team 筛选下拉、预算进度条、新鲜度徽章、骨架屏动画、管理员登录表单等。
...
- 控制台功能
  - 功能入口：用户映射管理、Team 月度账单、Cost Center 管理、返回用量看板。
...
**图表来源**
- [admin.html:184-205](file://public/admin.html#L184-L205)
...
## 依赖关系分析
- 页面到公共模块
  - script.js/analytics.js/billpage.js/costcenter.js/user.js/admin.js 均通过 C = CopilotDashboard 引用公共工具。
- 页面到后端路由
...
**更新** 新增的管理员控制台提供了完整的后台管理功能，包括安全的登录认证、会话管理和权限控制，确保只有授权用户才能访问管理功能。通过 requireAdminPage 和 requireAdminApi 中间件实现了前后端一致的权限控制策略，配合 bcrypt 密码哈希验证和会话固定攻击防护，构建了安全可靠的管理界面。

## 附录
- 设计规范与样式
  - 设计系统：主色、强调色、表面色、字体、阴影。
  - 组件：查询栏、表格、分页、模态框、Team 筛选、预算进度条、新鲜度徽章、骨架屏动画、管理员登录表单。
- API 一览（页面相关）
  - 主页面：/api/usage、/api/usage/refresh、/api/seats、/api/enterprise-teams、/api/enterprise-teams/:id/members、/api/billing/summary、/api/billing/models、/api/cost-centers。
  - 数据分析：/api/analytics/trends、/api/analytics/top-users、/api/analytics/daily-summary。
  - 月度账单：/api/bill、/api/bill/refresh。
