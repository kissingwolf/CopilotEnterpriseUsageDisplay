...
- 添加组件复用和样式管理的最佳实践指导
- **新增Copilot Insights页面文档**，包含AI洞察显示功能、来源徽章区分、AI状态信息展示等特性
...
## 简介
本设计文档聚焦 CopilotEnterpriseUsageDisplay 的前端页面，围绕主页面用量排行、数据分析、月度账单、成本中心、用户预算管理与用户映射六大页面，以及新增的Copilot Insights洞察页面，系统阐述功能设计、用户交互与数据可视化实现，并总结公共模块设计模式、IIFE 封装与命名空间管理，以及前端性能优化与组件复用最佳实践。

**更新** 新增用户预算管理页面，提供独立的前端界面、表单处理、模态对话框、实时数据过滤、视觉进度指示等特性。更新管理员控制台集成，添加直接导航链接，实现完整的后台管理功能。**新增Copilot Insights洞察页面**，支持AI驱动的动态洞察建议，提供来源徽章区分AI与规则建议，显示AI状态信息。
...
INSIGHTS["insights.html<br/>insights.js"]
...
- [insights.js:1-267](file://public/insights.js#L1-L267)
...
- [styles.css:1-1576](file://public/styles.css#L1-L1576)
...
- 页面脚本（IIFE）
  ...
  - **Copilot Insights洞察**：双Tab视图（IDE Usage + Code Generation）、AI洞察建议、来源徽章区分、AI状态显示。
  - 用户映射：Excel 上传、映射重载、成员列表、分页与排序、映射状态可视化。
  - 管理员控制台：登录认证、会话管理、权限控制、管理入口导航。
- 全局样式
  - 设计系统：星巴克风格绿为主色，暖中性背景，Serif 标题字体，Sans-serif 正文字体。
  - 组件：查询栏、表格、分页、模态框、Team 筛选下拉、预算进度条、新鲜度徽章、骨架屏动画、管理员登录表单、用户预算表单、**AI洞察来源徽章**。
...
## 架构总览
前端页面通过 IIFE 封装，依赖公共模块 CopilotDashboard 提供的工具函数；页面脚本通过 fetch 调用后端路由（/api/usage、/api/analytics/*、/api/bill、/api/cost-centers/*、/api/user/*、/api/user-budgets/*、/api/insights 等），实现数据驱动的渲染与交互。
...
- [insights.js:235-244](file://public/insights.js#L235-L244)
...
- [insights.js:108-126](file://routes/insights.js#L108-L126)
...
### Copilot Insights洞察页面：AI驱动的智能分析
- **双Tab视图设计**
  - IDE Usage Tab：展示Copilot IDE使用指标，包括活跃用户、Agent采纳率、聊天模型使用等。
  - Code Generation Tab：展示代码生成指标，包括代码变更行数、Agent贡献率、模型效率对比等。
- **AI洞察与建议系统**
  - **来源徽章区分**：通过紫色徽章标识"AI"生成的洞察，灰色徽章标识"规则"引擎生成的建议。
  - **AI状态显示**：在元数据区域显示AI状态（AI: on/AI: fallback），反映LLM服务可用性。
  - **智能优先级排序**：按严重级别（high/medium/low）自动排序，高优先级建议优先展示。
  - **证据与建议分离**：每条洞察包含具体证据数据和可执行的优化建议。
- **交互式图表展示**
  - 趋势图表：每日活跃用户、每周活跃用户、平均聊天请求等折线图。
  - 分布图表：聊天模型使用分布、语言使用分布、模型效率对比等饼图和柱状图。
  - 响应式设计：图表在不同屏幕尺寸下自适应调整大小。
- **数据聚合与缓存**
  - 实时数据获取：从GitHub Copilot Metrics Reports获取最新数据。
  - 智能回退机制：LLM服务不可用时自动降级为规则引擎结果。
  - 缓存策略：结合内存缓存和SQLite持久化，提升响应速度。

```mermaid
sequenceDiagram
participant U as "用户"
participant I as "insights.js"
participant R as "routes/insights.js"
participant LLM as "LLM服务"
U->>I : 访问/insights页面
I->>R : GET /api/insights?range=28
R->>R : 构建Insights数据负载
R->>LLM : generateLlmInsights(data)
alt LLM可用
LLM-->>R : 返回AI洞察建议
R->>R : mergeInsights(规则+AI建议)
else LLM不可用
LLM-->>R : 空数组(回退到规则引擎)
R->>R : 仅使用规则引擎建议
end
...
**图表来源**
- [insights.html:34-73](file://public/insights.html#L34-L73)
- [insights.js:206-233](file://public/insights.js#L206-L233)
- [insights.js:235-244](file://public/insights.js#L235-L244)
- [insights.js:108-126](file://routes/insights.js#L108-L126)
- [llm-insights.js:83-124](file://lib/llm-insights.js#L83-L124)
- [llm-insights.js:126-130](file://lib/llm-insights.js#L126-L130)

**章节来源**
- [insights.html:1-81](file://public/insights.html#L1-L81)
- [insights.js:1-267](file://public/insights.js#L1-L267)
- [insights.js:108-126](file://routes/insights.js#L108-L126)
- [llm-insights.js:1-139](file://lib/llm-insights.js#L1-L139)
- [insights-aggregator.js:530-646](file://lib/insights-aggregator.js#L530-L646)
- [styles.css:1383-1431](file://public/styles.css#L1383-L1431)
...
## 依赖关系分析
- 页面到公共模块
  - script.js/analytics.js/billpage.js/costcenter.js/user.js/userbudget.js/insights.js/admin.js 均通过 C = CopilotDashboard 引用公共工具。
- 页面到后端路由
  ...
  - **Copilot Insights**：/api/insights、/insights。
...
Insights --> InsightsRoute["routes/insights.js"]
Admin --> AuthRoute["routes/auth.js"]
Admin --> LibAuth["lib/auth.js"]
InsightsRoute --> LLM["lib/llm-insights.js"]
InsightsRoute --> Aggregator["lib/insights-aggregator.js"]
```
...
- [insights.js:4-4](file://public/insights.js#L4-L4)
...
- [insights.js:1-137](file://routes/insights.js#L1-L137)
- [auth.js:1-41](file://routes/auth.js#L1-L41)
- [lib/auth.js:1-62](file://lib/auth.js#L1-L62)
- [lib/llm-insights.js:1-139](file://lib/llm-insights.js#L1-L139)
- [lib/insights-aggregator.js:1-671](file://lib/insights-aggregator.js#L1-L671)
...
- **Copilot Insights优化**
  - **LLM服务超时保护**：默认15秒超时，避免长时间等待。
  - **智能回退机制**：LLM服务不可用时自动降级为规则引擎，确保用户体验。
  - **图表懒加载**：仅在Tab激活时渲染对应图表，减少初始加载时间。
  - **数据预聚合**：后端预先计算复杂指标，前端直接渲染。
...
- **Copilot Insights故障**
  - **LLM服务配置**：检查 LLM_URL、LLM_API_KEY、LLM_MODEL 环境变量配置。
  - **AI洞察不显示**：确认LLM服务可达，检查网络连通性和API密钥有效性。
  - **回退机制触发**：查看元数据区域的AI状态，确认是否回退到规则引擎。
  - **数据来源问题**：检查GitHub Copilot Metrics Reports API访问权限和数据完整性。
- 日志与可观测性
...
**更新** 新增的用户预算管理页面提供了完整的 SKU 级预算控制功能，包含实时过滤、模态对话框表单处理和预算进度可视化。管理员控制台集成了 User Budget 管理入口，通过 requireAdminPage 中间件实现统一的权限控制。**新增的Copilot Insights洞察页面**实现了AI驱动的智能分析功能，通过来源徽章清晰区分AI与规则建议，支持AI状态显示和智能回退机制。所有页面均采用一致的公共模块设计模式，确保代码复用和维护性。

## 附录
- 设计规范与样式
  - 设计系统：主色、强调色、表面色、字体、阴影。
  - 组件：查询栏、表格、分页、模态框、Team 筛选、预算进度条、新鲜度徽章、骨架屏动画、管理员登录表单、用户预算表单、**AI洞察来源徽章**。
- API 一览（页面相关）
  - 主页面：/api/usage、/api/usage/refresh、/api/seats、/api/enterprise-teams、/api/enterprise-teams/:id/members、/api/billing/summary、/api/billing/models、/api/cost-centers。
  - 数据分析：/api/analytics/trends、/api/analytics/top-users、/api/analytics/daily-summary。
  - 月度账单：/api/bill、/api/bill/refresh。
...
- **Copilot Insights配置**
  - LLM配置：LLM_URL、LLM_API_KEY、LLM_MODEL、LLM_ENABLED、LLM_TIMEOUT_MS。
  - 回退机制：LLM服务不可用时自动降级为规则引擎。
  - 数据源：GitHub Copilot Metrics Reports API。
...
- [lib/auth.js:39-62](file://lib/auth.js#L39-L62)
- [llm-insights.js:22-34](file://lib/llm-insights.js#L22-L34)
