...
- 环境变量作用与配置方法（GitHub API、缓存策略、调度器、日志级别、AI Credits 定价参数、管理员认证配置等）
...
A --> I["lib/auth.js<br/>管理员认证"]
A --> J["routes/billing.js<br/>账单路由"]
A --> K["routes/costcenter.js<br/>成本中心路由"]
A --> L["routes/user-mapping.js<br/>用户映射路由"]
A --> M["deploy/nginx-copilot-dashboard.conf<br/>Nginx 反代"]
A --> N["deploy/copilot-dashboard.service<br/>systemd 服务"]
```
...
- [lib/billing-config.js:1-83](file://lib/billing-config.js#L1-L83)
- [lib/helpers.js:1-83](file://lib/helpers.js#L1-L83)
- [lib/auth.js:1-120](file://lib/auth.js#L1-L120)
...
- 计费配置：计划类型、配额、单价与超量计费计算，新增 AI Credits 定价参数与管理员认证配置。
- 用户映射：本地 JSON 映射文件，支持热加载与文件变更监控。
- 辅助工具：查询参数构建、端点选择（企业/组织）、错误统一处理。
- 管理员认证：基于用户名和密码哈希的认证机制，支持安全的密码管理脚本。
...
- GitHub API 配置
  ...
  - GITHUB_API_VERSION：API 版本头，默认 2026-03-10；用于兼容新功能端点。
- 日志与运行时
...
- 计费与账单
  ...
  - BILLING_MODEL：计费模式，支持 "legacy_pru" 或 "ai_credits"。
  - AI_CREDIT_PRICE_FALLBACK：AI Credits 超量单价默认值，默认 0.01。
  - BUSINESS_SEAT_BASE_COST：业务计划座位基础价格，默认 19。
  - ENTERPRISE_SEAT_BASE_COST：企业计划座位基础价格，默认 39。
- 管理员认证
  - ADMIN_USER：管理员用户名。
...
**更新** 新增 AI Credits 定价参数（AI_CREDIT_PRICE_FALLBACK、BUSINESS_SEAT_BASE_COST、ENTERPRISE_SEAT_BASE_COST）和管理员认证配置（ADMIN_USER、ADMIN_PASSWORD_HASH）

章节来源
- [lib/github-api.js:24-27](file://lib/github-api.js#L24-L27)
- [lib/github-api.js:53-54](file://lib/github-api.js#L53-L54)
...
- [lib/billing-config.js:11-14](file://lib/billing-config.js#L11-L14)
- [lib/billing-config.js:47-51](file://lib/billing-config.js#L47-L51)
- [lib/auth.js:17-22](file://lib/auth.js#L17-L22)
...
**更新** 新增 AI Credits 定价参数配置选项，支持灵活的计费模式切换
...
### 管理员认证与安全配置
- 认证机制
  - 基于用户名和密码哈希的认证，支持通过 ADMIN_USER 和 ADMIN_PASSWORD_HASH 环境变量配置。
  - 密码哈希使用 bcrypt 算法，确保密码的安全存储。
- 密码管理
  - 提供专用脚本 scripts/hash-admin-password.js 生成 bcrypt 哈希值。
  - 支持交互式密码输入和命令行参数两种方式。
- 安全加固
  - 管理员界面需要有效的认证凭据才能访问。
  - 建议使用强密码并定期轮换管理员账户。
...
章节来源
- [lib/auth.js:17-22](file://lib/auth.js#L17-L22)
- [lib/auth.js:88-120](file://lib/auth.js#L88-L120)
- [scripts/hash-admin-password.js:1-30](file://scripts/hash-admin-password.js#L1-L30)
...
- AI Credits 定价优化
  - 合理设置 AI_CREDIT_PRICE_FALLBACK 参数，平衡成本与准确性。
  - 根据业务规模调整 BUSINESS_SEAT_BASE_COST 和 ENTERPRISE_SEAT_BASE_COST。

**更新** 新增 AI Credits 定价参数的性能考量
...
- 常见问题定位
  ...
  - 管理员认证失败：检查 ADMIN_USER 和 ADMIN_PASSWORD_HASH 配置是否正确。
- 资源释放与优雅停机
...
## 结论
本指南提供了 CopilotEnterpriseUsageDisplay 的全栈配置与定制方案，涵盖 GitHub API、缓存、调度器、日志、成本中心、用户映射、管理员认证与部署等关键方面。通过合理设置环境变量与缓存策略，结合调度器的回填机制与前端反代优化，可在不同环境中获得稳定、高效与可维护的运行体验。新增的 AI Credits 定价参数和管理员认证配置进一步增强了系统的灵活性与安全性。
...
- GitHub API
  ...
  - GITHUB_API_VERSION：默认 2026-03-10；用于兼容新功能端点。
- 日志与运行
...
- 计费与账单
  ...
  - BILLING_MODEL：计费模式，支持 "legacy_pru" 或 "ai_credits"。
  - AI_CREDIT_PRICE_FALLBACK：AI Credits 超量单价，默认 0.01。
  - BUSINESS_SEAT_BASE_COST：业务计划座位基础价格，默认 19。
  - ENTERPRISE_SEAT_BASE_COST：企业计划座位基础价格，默认 39。
- 管理员认证
  - ADMIN_USER：管理员用户名。
...
**更新** 新增 AI Credits 定价参数和管理员认证配置项
...
- 生产环境
  ...
  - 强制启用管理员认证，使用强密码和定期轮换策略。
  - 根据业务需求合理配置 AI Credits 定价参数。

**更新** 新增管理员认证在不同环境中的最佳实践
...
### 管理员密码哈希生成指南
- 使用专用脚本生成 bcrypt 哈希值
  - 命令行方式：node scripts/hash-admin-password.js 'your-strong-password'
  - 交互式方式：node scripts/hash-admin-password.js（会隐藏输入）
- 安全建议
  - 使用强密码（至少12位，包含大小写字母、数字和特殊字符）
  - 定期轮换管理员密码
  - 将生成的哈希值安全存储在 .env 文件中
