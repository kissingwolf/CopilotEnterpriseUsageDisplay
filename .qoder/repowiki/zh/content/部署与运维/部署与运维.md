...
## 更新摘要
**变更内容**
- 新增会话管理与安全认证机制，包括管理员认证、会话保护和 CSRF 防护
- 增强生产环境安全设置，涵盖 HTTPS 配置、安全头设置和会话存储
- 更新启动前检查脚本，增加会话配置验证和安全参数检查
- 完善部署配置，包含会话存储依赖和安全环境变量
...
C --> I["lib/auth.js<br/>管理员认证"]
B --> J["routes/auth.js<br/>认证路由"]
I --> K["bcrypt 密码哈希"]
I --> L["会话管理"]
M["scripts/preflight-check.*<br/>启动前自检"] -.-> A
N["deploy/nginx-copilot-dashboard.conf<br/>Nginx 反代"] --> A
O["deploy/copilot-dashboard.service<br/>systemd 服务单元"] --> A
P["CLAUDE.md<br/>项目指导"] --> A
Q["CONTEXT.md<br/>领域概念"] --> A
```
...
- [scripts/preflight-check.js:1-203](file://scripts/preflight-check.js#L1-L203)
...
- **新增** 会话管理与安全认证：基于 bcrypt 的密码验证、会话固定攻击防护、CSRF 防护。
...
AUTH["会话管理<br/>express-session"]
SEC["安全认证<br/>bcrypt + CSRF"]
...
### 会话管理与安全认证
- **管理员认证**：基于 bcrypt 的密码哈希验证，支持动态环境变量配置。
- **会话保护**：登录时进行会话 ID 重新生成，防止会话固定攻击。
- **CSRF 防护**：通过会话状态验证 API 请求的合法性。
- **会话存储**：依赖 express-session，需要配置合适的会话存储后端（如 Redis）。
- **安全头设置**：建议在 Nginx 层配置安全头，包括 X-Frame-Options、X-Content-Type-Options、Strict-Transport-Security。

```mermaid
flowchart TD
Start(["管理员登录"]) --> Validate["验证用户名密码<br/>bcrypt.compareSync"]
Validate --> |成功| Regenerate["会话ID重新生成<br/>regenerate()"]
Validate --> |失败| Deny["拒绝访问<br/>401 Unauthorized"]
Regenerate --> SetAdmin["设置 isAdmin=true<br/>保存用户信息"]
SetAdmin --> Success["登录成功<br/>返回用户信息"]
Deny --> End(["结束"])
Success --> End
```
...
- [routes/auth.js:25-40](file://routes/auth.js#L25-L40)
...
**图表来源**
- [scripts/preflight-check.js:71-202](file://scripts/preflight-check.js#L71-L202)
...
- **新增** 会话管理依赖 bcrypt 进行密码验证，依赖 express-session 进行会话存储。
- **新增** 认证路由依赖会话管理模块进行权限控制。
...
Auth --> RoutesAuth["routes/auth.js"]
Scheduler --> Store
GHAPI --> Store
UserMapping --> Store
```
...
- **新增** 会话存储优化：建议使用 Redis 等内存存储替代默认内存存储，提高会话性能。
...
- **新增** 会话管理问题
  - 检查会话存储配置（Redis/内存），确保会话正常读写。
  - 验证 bcrypt 密码哈希配置，确认 ADMIN_USER 和 ADMIN_PASSWORD_HASH 正确设置。
  - 检查 CSRF 防护配置，确保会话状态验证正常工作。
- **新增** 依赖注入问题
  - 确保所有路由模块正确接收依赖对象。
  - 检查工厂函数调用时的参数传递。
...
## 结论
通过 Nginx 反代与 systemd 管理，结合 pino 结构化日志与健康检查端点，CopilotEnterpriseUsageDisplay 可在生产环境稳定运行。借助启动前自检与自动刷新调度器，可有效降低权限与网络问题带来的风险；三层缓存与并发控制保障性能与可靠性。新增的会话管理与安全认证机制提供了企业级的安全保障，包括管理员认证、会话固定攻击防护和 CSRF 防护。基于 CLAUDE.md 的依赖注入模式和 CONTEXT.md 的领域概念词汇表，进一步提升了系统的可维护性和团队协作效率。建议在多实例部署中合理配置调度器，配合日志与健康检查实现可观测性与自动化运维。
...
- **新增** 配置会话存储（Redis/内存），安装 bcrypt 依赖
...
- LOG_LEVEL（可选）
- SCHED_*（调度器相关）
- **新增** ADMIN_USER、ADMIN_PASSWORD_HASH（管理员认证）
- **新增** SESSION_SECRET（会话密钥，用于会话加密）
...
### 生产环境安全配置
- **HTTPS 配置**：在 Nginx 中启用 TLS，配置强密码套件和安全头
- **会话安全**：设置安全的 SESSION_SECRET，配置会话过期时间和安全标志
- **CSRF 防护**：启用 CSRF 令牌验证，限制跨站请求
- **访问控制**：限制管理员路由访问，实施速率限制
- **日志审计**：记录安全事件和访问日志，设置适当的日志级别

**章节来源**
- [lib/auth.js:25-36](file://lib/auth.js#L25-L36)
- [routes/auth.js:25-40](file://routes/auth.js#L25-L40)
- [README.md:590-619](file://README.md#L590-L619)
...
- **新增** **会话管理**：使用 express-session 进行会话存储，bcrypt 进行密码验证

**章节来源**
- [CLAUDE.md:67-76](file://CLAUDE.md#L67-L76)
- [lib/auth.js:11-37](file://lib/auth.js#L11-L37)
...
- **认证端点**：/admin/login、/admin/logout、/admin/session（管理员认证相关）
