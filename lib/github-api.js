...
function getGithubApiVersion() {
  return process.env.GITHUB_API_VERSION && process.env.GITHUB_API_VERSION.trim()
    ? process.env.GITHUB_API_VERSION.trim()
    : "2026-03-10";
}
...
async function githubFetchRaw(method, pathname, searchParams, body, opts = {}) {
  ...
  try {
    ...
    const headers = {
      ...
      "X-GitHub-Api-Version": getGithubApiVersion(),
    };
...
async function githubPatchJson(pathname, body) {
  const data = await githubRequest("PATCH", pathname, undefined, body);
