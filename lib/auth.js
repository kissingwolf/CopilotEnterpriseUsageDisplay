...
function requireAdminMutation(req, res, next) {
  ...
  const isAdminOperation =
    /^\/api\/bill\/refresh\/?$/.test(pathname) ||
    /^\/api\/cost-centers(?:\/|$)/.test(pathname) ||
    /^\/api\/user-budgets(?:\/|$)/.test(pathname) ||
    /^\/user\/(?:upload-members|reload-mapping)\/?$/.test(pathname);
  if (!isAdminOperation) return next();
