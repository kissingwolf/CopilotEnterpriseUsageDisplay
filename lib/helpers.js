...
function buildQueryParams(extra = {}) {
  ...
  const now = new Date();
  const year = extra.year || requiredEnv("BILLING_YEAR") || String(now.getUTCFullYear());
  const month = extra.month || requiredEnv("BILLING_MONTH") || String(now.getUTCMonth() + 1);
...
function isCopilotBillingItem(item) {
  ...
  if (/seat|subscription|license|for[_ ]business|user[_ -]?months?/.test(sku) || /user[_ -]?months?/.test(unitType)) return false;
  return /premium[_ ]request|ai credits?|ai[_ ]unit|credit|token/.test(sku) || /requests?|credits?|tokens?|ai[_ -]units?/.test(unitType) || sku === "";
}
