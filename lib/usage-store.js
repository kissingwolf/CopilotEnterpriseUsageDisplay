  ...
  _prepareStatements() {
    this._stmts = {
      ...
      saveBillRow: this.db.prepare(`
        INSERT OR REPLACE INTO monthly_bill (year_month, team, login, plan_type, seat_cost, requests, quota, overage_requests, overage_cost, total_cost, computed_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `),
    ...
    const tx = this.db.transaction((items) => {
      ...
      for (const r of items) {
        this._stmts.saveBillRow.run(
          yearMonth, r.team, r.login, r.planType,
