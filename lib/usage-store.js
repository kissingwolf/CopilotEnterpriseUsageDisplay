...
const USAGE_TTL_MS = 72 * 60 * 60 * 1000; // 72 hours
...
class UsageStore {
  ...
  saveDay(dateStr, year, month, day, data, mode, raw_count, source, fetchedAt) {
    this.db.prepare(`
      INSERT OR REPLACE INTO daily_usage (date, year, month, day, data, mode, raw_count, source, fetched_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(dateStr, year, month, day, JSON.stringify(data), mode, raw_count, source, fetchedAt);
  }
