...
class UsageStore {
  ...
  _prepareStatements() {
    this._stmts = {
      ...
      deleteDaysInRange: this.db.prepare("DELETE FROM daily_usage WHERE date >= ? AND date <= ?"),
  ...
  restoreDaysInRange(startStr, endStr, rows) {
    const restore = this.db.transaction((snapshot) => {
      this._stmts.deleteDaysInRange.run(startStr, endStr);
    ...
    restore(Array.isArray(rows) ? rows : []);
  }
