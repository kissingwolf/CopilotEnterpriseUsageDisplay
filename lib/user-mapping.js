...
class UserMappingService {
  ...
  /**
   * Batch lookup: build a plain-object lookup for the given logins.
   * Returns { [normalizedLogin]: { adName, adMail, githubName, githubMail } }
   */
  buildLookup(logins) {
    const lookup = {};
    for (const login of logins) {
      ...
      const entry = this.userMap.get(normalized);
      if (entry) lookup[normalized] = entry;
    }
    return lookup;
  }
