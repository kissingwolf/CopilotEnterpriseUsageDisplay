...
// GitHub's 28-day metrics report buckets the long tail of (language|model) x feature
// combinations into a synthetic `others` feature paired with an `others`
// language/model. That catch-all aggregates non user-initiated activity
// (including agent_edit / copilot_cli deletions), so it must be excluded from
// the per-language / per-model breakdowns to match the GitHub UI caliber.
function isOthersFeature(feature) {
  return String(feature || "").toLowerCase() === "others";
}

// GitHub's report attributes some chat activity (mostly agent-mode tool calls)
// to a synthetic `unknown` model. GitHub's "Chat model usage" / "Most used chat
// model" surfaces only real, attributable models, so this catch-all must be
// excluded to avoid it winning the ranking.
function isUnknownModel(model) {
  return String(model || "").toLowerCase() === "unknown";
}
...
function buildReportCodeChangesByModel(dayTotals, agentInitiated) {
  ...
  for (const day of Array.isArray(dayTotals) ? dayTotals : []) {
    for (const item of Array.isArray(day.totals_by_model_feature) ? day.totals_by_model_feature : []) {
      ...
      if (agentInitiated !== isAgent || isCopilotCliFeature(item.feature) || isOthersFeature(item.feature)) continue;
...
function buildReportChatModelUsage(dayTotals) {
  ...
  for (const day of Array.isArray(dayTotals) ? dayTotals : []) {
    for (const item of Array.isArray(day.totals_by_model_feature) ? day.totals_by_model_feature : []) {
      if (isOthersFeature(item.feature)) continue;
      if (isUnknownModel(item.model)) continue;
      const model = item.model || "Other models";
...
function generateInsightRecommendations(summary) {
  ...
  const activeUsers = summary.agentAdoption?.activeUsers || summary.ideActiveUsers || 0;
  const agentContribution = summary.agentContribution || 0;
  const averageLinesDeletedByAgent = summary.averageLinesDeletedByAgent || 0;
  const modelEfficiency = summary.modelEfficiency || [];
  const agentModels = modelEfficiency.filter((model) => model.mode === "agent");
  const languageUsage = summary.languageUsage || [];
  const chatModelUsage = summary.chatModelUsage || [];
  const completionAcceptanceRate = summary.completionAcceptanceRate || 0;
  const completionSuggestions = summary.completionSuggestions || 0;
  const linesChangedWithAi = summary.linesChangedWithAi || 0;

  // 1. ROI 高价值警报 — 深度 Agent 研发模式
  ...
  // 2. Agent 采纳率偏低 — 能力未规模化落地
  if (activeUsers > 0 && agentAdoption > 0 && agentAdoption < 30) {
    recommendations.push({
      type: "adoption-gap",
      title: "Agent 采纳率偏低",
      severity: "medium",
      message: "多数活跃用户仍以补全/对话为主，Agent 能力尚未规模化落地。",
      evidence: `Agent adoption ${agentAdoption}%（活跃用户 ${activeUsers}）。`,
      recommendation: "建议开展 Agent 模式实操培训与内部案例分享，降低上手门槛。",
    });
  }

  // 3. 代码资产重构评估 — 大量删除即重构信号
  ...
  // 5. 补全接受率偏低 — 上下文或建议质量影响信任度
  if (completionSuggestions >= 50 && completionAcceptanceRate > 0 && completionAcceptanceRate < 20) {
    recommendations.push({
      type: "completion-acceptance-low",
      title: "补全接受率偏低",
      severity: "medium",
      message: "代码补全接受率偏低，提示上下文或建议质量可能影响开发者信任度。",
      evidence: `Completion acceptance rate ${completionAcceptanceRate}%（suggestions ${completionSuggestions}）。`,
      recommendation: "建议完善 .github/copilot-instructions.md 上下文，并针对高频场景优化提示词。",
    });
  }

  // 6. 模型集中度偏高 — 单点依赖与成本波动风险
  const topChatModel = chatModelUsage[0];
  if (topChatModel && topChatModel.percent > 60) {
    recommendations.push({
      type: "model-concentration",
      title: "模型集中度偏高",
      severity: "medium",
      message: `聊天请求高度集中于 ${topChatModel.label}，存在单点依赖与成本波动风险。`,
      evidence: `${topChatModel.label} 占比 ${topChatModel.percent}%。`,
      recommendation: "建议按任务类型评估多模型分配策略，平衡成本、时延与质量。",
    });
  }

  // 7. 补全接受率优秀 — 可作为最佳实践样板
  if (completionAcceptanceRate >= 35) {
    recommendations.push({
      type: "completion-acceptance-high",
      title: "补全接受率优秀",
      severity: "low",
      message: "代码补全接受率处于高位，开发者对 AI 建议信任度高。",
      evidence: `Completion acceptance rate ${completionAcceptanceRate}%。`,
      recommendation: "建议沉淀当前高接受率团队的最佳实践，作为其他团队推广样板。",
    });
  }
  ...
  // 8. 语言集中度偏高 — 单一语言主导
  const topLanguage = languageUsage[0];
  if (topLanguage && topLanguage.percent > 50) {
    recommendations.push({
      type: "language-concentration",
      title: "语言集中度偏高",
      severity: "low",
      message: `Copilot 使用高度集中在 ${topLanguage.label}，其他技术栈覆盖不足。`,
      evidence: `${topLanguage.label} 占比 ${topLanguage.percent}%。`,
      recommendation: "建议针对主力语言沉淀提示词模板，并评估在其他语言场景推广采纳。",
    });
  }

  // 9. 技术栈失衡提示 — 核心语言高、TypeScript 低
  ...
  // 10. AI 规模化产出治理 — 变更量达规模化水平
  if (linesChangedWithAi > 100000) {
    recommendations.push({
      type: "scaled-output",
      title: "AI 规模化产出治理",
      severity: "low",
      message: "AI 参与的代码变更量已达规模化水平，需要配套质量与安全治理。",
      evidence: `Lines changed with AI ${linesChangedWithAi}。`,
      recommendation: "建议强化 AI 生成代码的评审、测试覆盖与安全扫描门禁。",
    });
  }
...
function buildInsightsPayload(rawSources = {}, options = {}) {
  ...
  const completionSummary = breakdown.length > 0 ? summarizeCompletions(breakdown) : reportCompletionAcceptance.summary;
  const ideActiveUsers = toNumber(usage.total_active_users) || toNumber(latestDay.monthly_active_users) || toNumber(latestDay.daily_active_users);
  const insights = generateInsightRecommendations({
    ...
    ideActiveUsers,
    completionAcceptanceRate: completionSummary.rate,
    completionSuggestions: completionSummary.suggestions,
    linesChangedWithAi,
  });
