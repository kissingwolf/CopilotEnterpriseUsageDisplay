...
- **Doughnut 饼图** — 使用 Chart.js 将用户按 `lastActivityAt` 自动归入 4 个分类：**1~5日不活跃**（绿）、**6~10日不活跃**（黄）、**10日以上不活跃**（红）、**注册后未活跃**（灰）；Tooltip 同时显示人数与百分比。
- **Team 单选过滤** — Tab 顶部提供"选择 Team"下拉菜单，从 `/api/user/members` 返回值中提取唯一 Team 列表；切换 Team 实时重算饼图，无需重新请求接口。
- **点击展开用户列表** — 点击饼图扇形后，在图表下方展开对应分类的用户列表（3 列：显示名 / Team / 最后活跃）；再次点击同一扇形收起列表。用户名遵循全局映射规则（已映射显示 AD 用户名，未映射显示 GitHub 用户名）。
- **复用现有接口** — 前端直接调用 `/api/user/members`，无需新增后端路由。
- **纯函数抽取与单元测试** — 分类逻辑提取为 `lib/helpers.js` 中的 `classifyUserActivity(members, nowMs)` 纯函数，新增 10 个单元测试，全项目测试数量提升至 49 个。
