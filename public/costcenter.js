(function () {
  ...
  var detailName = pathParts.length >= 2 && pathParts[0] === "costcenter" ? decodeURIComponent(pathParts.slice(1).join("/")) : "";
  var currentDetailCostCenter = null;
  var cachedEnterpriseTeams = null;

  if (backLink) {
    if (detailName) { backLink.href = "/costcenter"; backLink.textContent = "返回Cost Center 管理页"; }
    else { backLink.href = "/"; backLink.textContent = "返回用量看板"; }
  }
  ...
  function buildUserCount(resources) {
    var users = 0;
    (resources || []).forEach(function (r) { if (String(r.type || "").toLowerCase() === "user") users += 1; });
    return users;
  }

  function buildResourceCount(resources, type) {
    var target = String(type || "").toLowerCase();
    return (resources || []).filter(function (r) { return String(r.type || "").toLowerCase() === target; }).length;
  }

  function groupResources(resources) {
    var groups = { user: [], organization: [], repository: [], other: [] };
  ...
  function buildResourceDetails(resources) {
    ...
    var sections = [
      { key: "user", title: "Users" }, { key: "organization", title: "Organizations" },
      { key: "repository", title: "Repositories" }, { key: "other", title: "Others" },
    ];
    var html = '<div class="cc-resource-grid">';
    sections.forEach(function (section) {
      var list = groups[section.key];
      html += '<div class="cc-resource-group"><div class="cc-resource-title">' + section.title + ' (' + list.length + ')</div>';
  ...
  function formatBudgetCell(amount, spentAmount) {
    ...
    if (spentNum == null) return '<div class="budget-cell"><span class="budget-spent">-- spent</span><span class="budget-total">' + C.formatUsd(budgetNum) + ' budget</span></div>';
    ...
    return '<div class="budget-cell budget-progress-cell">' +
      ...
      '<div class="budget-bar"><div class="budget-bar-fill budget-' + level + '" style="width:' + width.toFixed(1) + '%"></div></div>' + over + '</div>';
  }
  ...
  function buildDetailInfo(cc) {
    ...
    return '<div class="cc-info-grid">' +
      '<div class="cc-info-item"><span class="k">ID</span><span class="v">' + C.escapeHtml(cc.id || "-") + '</span></div>' +
      '<div class="cc-info-item"><span class="k">名称</span><span class="v">' + C.escapeHtml(cc.name || "-") + '</span></div>' +
      '<div class="cc-info-item"><span class="k">状态</span><span class="v">' + C.escapeHtml(cc.state || "-") + '</span></div>' +
      '<div class="cc-info-item"><span class="k">Azure Subscription</span><span class="v">' + C.escapeHtml(cc.azureSubscription || "-") + '</span></div>' +
      ...
      '<div class="cc-info-item"><span class="k">预算进度</span><span class="v">' + formatBudgetCell(cc.budgetAmount, cc.spentAmount) + '</span></div></div>';
  }

  function renderList(data) {
    var rows = Array.isArray(data.costCenters) ? data.costCenters : [];
    var seatBaseCost = C.toNumber(data && data.seatBaseCost);
    meta.textContent = "Enterprise: " + (data.enterprise || "-") + " | 总数: " + (data.total || 0) + " | 最后刷新: " + C.formatTs(data.fetchedAt);
    latestMetaText = meta.textContent;
    if (!rows.length) { tbody.innerHTML = '<tr><td colspan="5" class="empty">当前筛选下没有 cost center 数据。</td></tr>'; return; }
    tbody.innerHTML = "";
    var index = 0;
    function renderChunk() {
      ...
      for (; index < end; index += 1) {
        ...
        html += "<tr>" +
          '<td><div class="cc-name-wrap"><a class="cc-name-link" href="' + detailLink + '">' + C.escapeHtml(row.name || "-") + '</a>' + buildExpandBtn(detailId, false) + '</div></td>' +
          ...
          "<td>" + C.escapeHtml(row.state || "-") + "</td><td>" + buildUserCount(row.resources) + "</td></tr>";
        html += '<tr id="' + detailId + '" class="cc-detail-row" hidden><td colspan="5">' + buildResourceDetails(row.resources) + "</td></tr>";
      }
  ...
  function renderDetail(data) {
    ...
    if (!cc) { tbody.innerHTML = '<tr><td colspan="5" class="empty">未找到该 cost center。</td></tr>'; return; }
    pageTitle.textContent = "Cost Center 详情";
    currentDetailCostCenter = cc;
    if (teamAssignPanel) teamAssignPanel.hidden = false;
    meta.textContent = "Enterprise: " + (data.enterprise || "-") + " | 名称: " + (cc.name || "-") + " | 最后刷新: " + C.formatTs(data.fetchedAt);
    latestMetaText = meta.textContent;
    var detailId = "cc-detail-single";
    var html = "<tr>" +
      '<td><div class="cc-name-wrap">' + C.escapeHtml(cc.name || "-") + buildExpandBtn(detailId, true) + "</div></td>" +
      ...
      "<td>" + C.escapeHtml(cc.state || "-") + "</td><td>" + buildUserCount(cc.resources) + "</td></tr>";
    html += '<tr id="' + detailId + '" class="cc-detail-row"><td colspan="5">' + buildDetailInfo(cc) + buildResourceDetails(cc.resources) + "</td></tr>";
    tbody.innerHTML = html;
    loadTeamOptions().catch(function (err) { setError(err instanceof Error ? err.message : String(err)); });
  }

  async function loadTeamOptions() {
    if (!detailName || !ccTeamList) return;
    if (!cachedEnterpriseTeams) {
      var data = await C.apiFetchJson("/api/enterprise-teams", {}, "获取 Team 失败");
      cachedEnterpriseTeams = Array.isArray(data.teams) ? data.teams : [];
    }
    ccTeamList.innerHTML = cachedEnterpriseTeams.map(function (t) {
      return '<label class="cc-team-item"><input type="checkbox" class="cc-team-cb" value="' + String(t.id) + '" /><span class="name">' + C.escapeHtml(t.name || "-") + '</span><span class="desc">' + C.escapeHtml(t.description || "") + '</span></label>';
    }).join("");
    updateTeamAllState();
  }
  ...
  function getSelectedTeamIds() {
    ...
    var ids = [];
    boxes.forEach(function (b) { if (b.checked) ids.push(String(b.value)); });
    return ids;
  }
  function updateTeamAllState() {
    if (!ccTeamAll || !ccTeamList) return;
    var boxes = ccTeamList.querySelectorAll(".cc-team-cb");
    var total = boxes.length, checked = getSelectedTeamIds().length;
  ...
  function renderAssignResult(payload) {
    ...
    ccAssignResult.innerHTML =
      ...
      (existing.length ? '<details><summary>已存在用户 (' + existing.length + ')</summary><div class="cc-user-list">' + existing.map(C.escapeHtml).join(", ") + '</div></details>' : "") +
      (newcomers.length ? '<details open><summary>将新增用户 (' + newcomers.length + ')</summary><div class="cc-user-list">' + newcomers.map(C.escapeHtml).join(", ") + '</div></details>' : "") +
      (removals.length ? '<details><summary>可删除用户（Cost Center 有 / Team 无）(' + removals.length + ')</summary><div class="cc-user-list">' + removals.map(C.escapeHtml).join(", ") + '</div></details>' : "");
  }

  async function runTeamAssign(dryRun, removeMissingUsers) {
    if (!currentDetailCostCenter || !currentDetailCostCenter.id) throw new Error("当前 cost center 信息不完整。");
    var teamIds = getSelectedTeamIds();
    if (!teamIds.length) throw new Error("请至少选择一个 Team。");
    if (!dryRun) { if (!window.confirm("确认将所选 Team 成员批量加入当前 cost center 的 Users 吗？")) return null; }
    return C.apiFetchJson(
      "/api/cost-centers/" + encodeURIComponent(currentDetailCostCenter.id) + "/add-users-from-teams",
      { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ teamIds: teamIds, dryRun: dryRun, removeMissingUsers: Boolean(removeMissingUsers) }) },
      "批量加入失败"
    );
  }
  ...
  async function refresh(options) {
    ...
    var cached = C.getCachedData(cacheKey, CC_CACHE_TTL_MS);
    if (cached) { if (detailName) renderDetail(cached); else renderList(cached); }
    else C.renderSkeletonRows(tbody, 5, detailName ? 4 : 8);
    ...
    try {
      var data;
      if (detailName) data = await C.apiFetchJson("/api/cost-centers/by-name/" + encodeURIComponent(detailName), {}, "获取 cost center 详情失败");
      else { var state = stateSel.value; var query = state ? ("?state=" + encodeURIComponent(state)) : ""; data = await C.apiFetchJson("/api/cost-centers" + query, {}, "获取 cost center 列表失败"); }
      if (detailName) renderDetail(data); else renderList(data);
      C.setCachedData(cacheKey, data);
    } catch (err) { setError(err instanceof Error ? err.message : String(err)); }
    finally { refreshBtn.disabled = false; refreshBtn.textContent = oldText; setMetaRefreshing(false); }
  }

  refreshBtn.addEventListener("click", function () { refresh(); });
  if (!detailName) stateSel.addEventListener("change", function () { refresh(); });
  else stateSel.disabled = true;
  ...
  if (ccTeamAll) ccTeamAll.addEventListener("change", function () {
    var boxes = ccTeamList ? ccTeamList.querySelectorAll(".cc-team-cb") : [];
    boxes.forEach(function (b) { b.checked = ccTeamAll.checked; });
    updateTeamAllState();
  });
  if (ccTeamList) ccTeamList.addEventListener("change", updateTeamAllState);

  if (ccPreviewBtn) ccPreviewBtn.addEventListener("click", async function () {
    setError(""); try { ccPreviewBtn.disabled = true; ccApplyBtn.disabled = true; var result = await runTeamAssign(true, false); if (result) renderAssignResult(result); }
    catch (err) { setError(err instanceof Error ? err.message : String(err)); }
    finally { ccPreviewBtn.disabled = false; ccApplyBtn.disabled = false; }
  });

  if (ccApplyBtn) ccApplyBtn.addEventListener("click", async function () {
    setError(""); try {
      ccApplyBtn.disabled = true; ccPreviewBtn.disabled = true;
      ...
      if (preview && Array.isArray(preview.usersToRemove) && preview.usersToRemove.length > 0) {
        shouldRemove = window.confirm("发现 Cost Center 中存在 Team 中没有的用户（" + preview.usersToRemove.length + " 个）。\n\n点击\"确定\"=确认删除这些用户\n点击\"取消\"=忽略本次删除，仅新增缺失用户");
      }
      var result = await runTeamAssign(false, shouldRemove);
      if (result) renderAssignResult(result);
      await refresh();
    } catch (err) { setError(err instanceof Error ? err.message : String(err)); }
    finally { ccApplyBtn.disabled = false; ccPreviewBtn.disabled = false; }
  });

  C.renderSkeletonRows(tbody, 5, detailName ? 4 : 8);
