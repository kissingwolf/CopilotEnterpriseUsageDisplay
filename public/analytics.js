...
async function apiFetchJson(url, options, fallbackMessage) {
  ...
  if (!resp.ok || (data && data.ok === false)) {
    throw new Error((data && data.message) || fallbackMessage || "请求失败");
  }
  return data;
}
...
function renderSummaryCards(data) {
  ...
  summaryCards.innerHTML = cards.map(function (c) {
    return '<div class="summary-card"><div class="label">' + escapeHtml(c.label) + '</div><div class="value">' + c.value + '</div></div>';
  }).join("");
}
...
async function refresh() {
  ...
  try {
    var [summary, trend, top] = await Promise.all([
      apiFetchJson("/api/analytics/daily-summary?range=" + currentRange, {}, "获取汇总数据失败"),
      apiFetchJson("/api/analytics/trends?range=" + currentRange, {}, "获取趋势数据失败"),
      apiFetchJson("/api/analytics/top-users?range=" + currentRange, {}, "获取Top用户失败"),
    ]);
    ...
    var now = new Date();
    meta.textContent = "数据范围: " + currentRange + " 天 | 最后加载: " + now.toLocaleString("zh-CN", { hour12: false });
  } catch (err) {
